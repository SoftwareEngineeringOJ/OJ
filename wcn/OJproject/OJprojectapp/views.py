...
def problemss(req):
    ...
    if req.POST:
        if cmp(req.POST["oj"],"all") == 0:
            problems_list=problemslist.objects.all().order_by('-id')
        else:
            problems_list=problemslist.objects.filter(OJ=req.POST["oj"]).order_by('-id')
        find=oj_show.index(req.POST["oj"])
        first=oj_show[0]
        oj_show[0]=req.POST["oj"]
        oj_show[find]=first
        if cmp(req.POST["sid"],"") != 0:
            problems_list=problems_list.filter(SID=req.POST["sid"]).order_by('-id')
        sid_show=req.POST["sid"]
        if cmp(req.POST["title"],"") != 0:
            problems_list=problems_list.filter(title=req.POST["title"]).order_by('-id')
        title_show=req.POST["title"]
        if cmp(req.POST["source"],"") != 0:
            problems_list=problems_list.filter(source=req.POST["source"]).order_by('-id')
        ...
        problems_list=problemslist.objects.all().order_by('-id')[0:1000]
        sid_show=""
        title_show=""
        source_show=""
        value1="1000"
...
def statuss(req):
    ...
    if req.POST:
        ...
        status_list=status.objects.all().order_by('-id')[0:1000]
        user_show=""
        value1="1000"
        value2="2000"
        value3="3000"
