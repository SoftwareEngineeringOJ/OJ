...
def index(req):
    ...
    return render_to_response('oj.html',{'username' : username, 
                                           'Flag' : Flag,'rank_list':rank_list},context_instance=RequestContext(req))
...
def modify(req):
    ...
    if req.method == 'POST':
        ...
        uf = UserRegisterForm()
    return render_to_response('modify.html',{'uf':uf, 'user':user_}, context_instance=RequestContext(req))
...
def enter(req):
    ...
    if cmp("",username)==0:
        ...
        ac = theuser.problems.all().filter(result='Accepted').order_by('-submit_time')
