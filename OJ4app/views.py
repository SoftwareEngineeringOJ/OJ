...
def oj(req):
    if req.method == 'POST':
        user_list = user.objects.all().order_by('right')
        user_list = user_list[0:10]
        i = 0
        rank_list = []
        for member in user_list:
            i = i + 1 
            rank_list.append([i,user_list[i-1].username])
        if i < 10:
        ...
        if i < 10:
           for j in range(i+1,11):
               rank_list.append([j,''])
    return render_to_response('oj.html',{'rank_list':rank_list},context_instance=RequestContext(req))
...
def regist(req):
    if req.method == 'POST':
        ...
        if uf.is_valid():
            ...
            #添加到数据库
            user.objects.create(username= username,password=password,signature=signature,school=school,email=email,register_time=datetime.datetime.now(),answered=0,right=0,wrong=0,contest_number=0,contest_right=0,contest_wrong=0)
...
def myoj(req):
    ...
    if req.method == 'POST':
        ...
        if i < 10:
           for j in range(i+1,11):
               rank_list.append([j,''])
    return render_to_response('myoj.html',{'username':username,'rank_list':rank_list})
...
def myproblemshow(req):
    ...
    problem = problemslist.objects.get(id=choice)
    pic_addr = problem.pic.split()
    return render_to_response('myproblemshow.html',{'problem':problem,'username':username,'pic_addr':pic_addr},context_instance=RequestContext(req))
def myusershow(req):
    choice = req.GET["id"]
    auser = user.objects.get(id=choice)
...
def mysubmitcode(req):
    ...
    if req.POST:
        ...
        sharecode =req.POST["sharecode"]
        shareuser =req.POST["shareuser"]
...
def mydiscuss(req):
    ...
    if req.POST:
        ...
        discussion.objects.create(userID= auser.id,username=username,problemID=problemid,problemTitle=problem.title,speak=speak,submit_time=datetime.datetime.now(),like=0)
    discussion_list = discussion.objects.filter(problemID=problemid)

    return render_to_response('mydiscussion.html',{'problemid':problemid,'username':username,'discussion_list':discussion_list,'problem':problem},context_instance=RequestContext(req))
